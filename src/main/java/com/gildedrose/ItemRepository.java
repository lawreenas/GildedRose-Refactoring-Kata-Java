...
public class ItemRepository {
    ...
    Item[] items = new Item[] {
            new GenericItemItem("+5 Dexterity Vest", 10, 20), //
            new AgedBrieItem("Aged Brie", 2, 0), //
            new GenericItemItem("Elixir of the Mongoose", 5, 7), //
            ...
            new GenericItemItem("Conjured Mana Cake", 3, 6) };
    ...
    public Item findById(String uuid) {
        return Arrays.stream(items).filter(item -> ((GenericItemItem) item).id.equals(uuid)).findFirst().get();
    }
}
